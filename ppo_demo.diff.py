...
def test_env(env, model, device):
    ...
    while not done:
        ...
        next_frame = grey_crop_resize(next_frame)
...
def ppo_train(model, envs, device, optimizer, test_rewards, test_epochs, train_epoch, best_reward, early_stop=False):
    ...
    frame = envs.reset()
    frame = grey_crop_resize_batch(frame)
    state = frame - frame
    ...
    while not early_stop:
        ...
        for i in range(T):
            ...
            next_frame, reward, done, _ = envs.step(action.cpu().numpy())
            next_frame = grey_crop_resize_batch(next_frame)  # simplify perceptions (grayscale-> crop-> resize) to train CNN
            next_state = next_frame - frame
