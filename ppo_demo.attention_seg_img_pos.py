...
H_SIZE = 16 # hidden size, linear units of the output layer
...
MODEL_DIR = 'models'
MODEL = f'ppo_demo.seg_img_pos_no_pos_EMBED_DIM_{EMBED_DIM}'
# ENV_ID = 'Pong-v0'
...
class ActorCritic(nn.Module):
    def __init__(self, num_inputs, num_outputs, hidden_size=H_SIZE, embed_dim=EMBED_DIM):
        ...
        self.attention = nn.Sequential(
            MyTransformerEncoderLayer(embed_dim, embed_dim // 4, embed_dim * 2),
            MyTransformerEncoderLayer(embed_dim, embed_dim // 4, embed_dim * 2),
        )
        self.feature = nn.Conv2d(NUM_Y * NUM_X, 1, kernel_size=1)
        ...
        self.actor = nn.Sequential(  # The “Actor” updates the policy distribution in the direction suggested by the Critic (such as with policy gradients)
            nn.Linear(in_features=hidden_size, out_features=hidden_size * 2),
            nn.ReLU(),
            nn.Linear(in_features=hidden_size * 2, out_features=num_outputs),
            nn.Softmax(dim=1),
        )
    ...
    def forward(self, x):
        ...
        patches_feature = patches_feature.view(x.shape[0], NUM_Y * NUM_X, -1)  # (B, n, embed_dim)
        patches_feature = patches_feature.permute(1, 0, 2)  # (n, B, embed_dim)
        attention = self.attention(patches_feature)  # (n, B, embed_dim)
