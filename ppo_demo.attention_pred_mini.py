...
MODEL_DIR = 'models'
MODEL = f'ppo_demo.attention_pred_mini_{LOOK_BACK_SIZE}_c3_{C_3}'
# ENV_ID = 'Pong-v0'
...
class ActorCritic(nn.Module):
    def __init__(self, num_inputs, num_outputs, hidden_size=H_SIZE, embed_dim=EMBED_DIM):
        ...
        self.feature = nn.Sequential(OrderedDict([
            ...
            ('act2', nn.ReLU()),
            ('conv3', nn.Conv2d(in_channels=32, out_channels=16, kernel_size=3, stride=2)),
            ('act3', nn.ReLU()),
            ('flatten', nn.Flatten()),
            ('linear', nn.Linear(in_features=16 * 4 * 4, out_features=hidden_size)),
            ('softmax', nn.Softmax(dim=-1)),
        ]))

        # self.attention = nn.MultiheadAttention(embed_dim, 16)
        self.attention = MyTransformerEncoderLayer(embed_dim, 16, 64)
        # self.pos_encode = PositionalEmbedding(EMBED_DIM, LOOK_BACK_SIZE)

        self.pred_next = nn.Sequential(
            nn.Linear(in_features=hidden_size + num_outputs, out_features=hidden_size),  # logits
        )
